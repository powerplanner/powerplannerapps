...
namespace PowerPlannerAndroid.Views.Controls
{
    public class TaskProgressBarControl : InflatedViewWithBinding, SeekBar.IOnSeekBarChangeListener
    {
        ...
        public TaskProgressBarControl(ViewGroup root) : base(Resource.Layout.TaskProgressBarControl, root)
        {
            Initialize();
        }
        ...
        {
            _seekBar = FindViewById<SeekBar>(Resource.Id.SeekBarTaskProgress);
            _seekBar.SetOnSeekBarChangeListener(this);

            _imageButtonDone = FindViewById<ImageButton>(Resource.Id.ImageButtonTaskProgressDone);
            _imageButtonDone.Click += _imageButtonDone_Click;

            _imageButtonIncomplete = FindViewById<ImageButton>(Resource.Id.ImageButtonTaskProgressIncomplete);
        ...
        public void OnProgressChanged(SeekBar seekBar, int progress, bool fromUser)
        {
            if (progress >= 100)
            {
                ViewCompat.SetBackgroundTintList(_imageButtonDone, ContextCompat.GetColorStateList(Context, Resource.Color.task_progress_complete_button_background_completed));
                ViewCompat.SetBackgroundTintList(_imageButtonIncomplete, ContextCompat.GetColorStateList(Context, Resource.Color.task_progress_button_background_unselected));
            }

            else
            {
                ViewCompat.SetBackgroundTintList(_imageButtonDone, ContextCompat.GetColorStateList(Context, Resource.Color.task_progress_button_background_unselected));
                ViewCompat.SetBackgroundTintList(_imageButtonIncomplete, ContextCompat.GetColorStateList(Context, Resource.Color.task_progress_incomplete_button_background));
            }
        }
