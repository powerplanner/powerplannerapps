...
namespace InterfacesUWP.CalendarFolder
{
    public class DefaultSmallCalendarSquare : TCalendarSquare
    {
        ...
        {
            base.Margin = new Microsoft.UI.Xaml.Thickness(1);

            _container = new Grid();
            base.Children.Add(_container);

            _container.RowDefinitions.Add(new Microsoft.UI.Xaml.Controls.RowDefinition() { Height = new GridLength(1, GridUnitType.Star) });
            _container.RowDefinitions.Add(new Microsoft.UI.Xaml.Controls.RowDefinition() { Height = GridLength.Auto });
        ...
        {
            return new SolidColorBrush(Windows.UI.Color.FromArgb(255, val, val, val));
        }
        ...
        protected override void SetBackgroundColor(TCalendarSquare.DisplayType type, bool isMouseHovering, bool isMouseDown)
        {
            if (type == DisplayType.ThisMonthSelected || type == DisplayType.OtherMonthSelected || type == DisplayType.TodaySelected)
                setHighlightVisibility(Microsoft.UI.Xaml.Visibility.Visible);
            else
                setHighlightVisibility(Microsoft.UI.Xaml.Visibility.Collapsed);
